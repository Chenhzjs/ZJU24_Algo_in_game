...
void DrawArc(double r1, double r2, double start, double sweep)
{
    DrawEllipticalArc(r1, r2, start, sweep);
}
...
void DrawTextString_Middle(string text)
{
    InitCheck();
    if (regionState != NoRegion) {
        Error("Text strings are illegal inside a region");
    }
    cx -= TextStringWidth(text) / 2;
    DisplayText(cx, cy, text);
    cx += TextStringWidth(text);
}
void DrawTextString_Right(string text)
{
    InitCheck();
    if (regionState != NoRegion) {
        Error("Text strings are illegal inside a region");
